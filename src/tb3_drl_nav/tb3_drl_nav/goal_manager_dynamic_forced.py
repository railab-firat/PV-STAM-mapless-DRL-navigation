#!/usr/bin/env python3
"""
goal_manager_dynamic_forced.py  [Task 8] lstm_forced curriculum variant
=====================================================================
Copy of goal_manager_dynamic.py with ONE behavioral change: phase advance
is gated on a fixed per-phase episode budget (FORCED_PHASE_BUDGET) instead
of the SR threshold, and never regresses. All 3 SR-gated LSTM seeds stalled
at Phase 4 (obstacle-free) and never saw a dynamic obstacle — this variant
forces progression through Phases 5-7 so the LSTM-vs-attention comparison
is actually about architecture, not about curriculum exposure.
Everything else (goal pools, spawn safety, CSV logging, resume) is identical
to goal_manager_dynamic.py.
=====================================================================
...
# [Task 8 lstm_forced] per-phase episode budget — total 3000, matching the
# project's standard journal-experiment budget (CLAUDE.md). Static phases
# 1-4 get 1100 eps combined (generous, since LSTM converges slower than
# the attention variants); phases 5-7 get 1900 combined to guarantee real
# exposure to dynamic obstacles. Override any phase via env e.g.
# FORCED_BUDGET_P5=500.
FORCED_PHASE_BUDGET = {
    p: int(os.environ.get(f"FORCED_BUDGET_P{p}", d))
    for p, d in {1: 300, 2: 200, 3: 250, 4: 350, 5: 600, 6: 650, 7: 650}.items()
}
...
class GoalManagerDynamic(Node):
    def __init__(self):
        ...
        # [Task 8 lstm_forced] _load_saved_phase() may already have restored this
        # from disk (it runs earlier in __init__) — do NOT clobber it, or a restart
        # silently grants the current phase its full budget a second time.
        if not hasattr(self, "_phase_ep_count"):
    ...
    def _on_step_result(self, msg: Float32MultiArray):
        ...
        if done and self._ep_open:
            ...
            if not self._eval_mode:
                # [Task 8 lstm_forced] Curriculum advances on a fixed per-phase
                # episode budget instead of the SR gate — SR-gated LSTM stalled
                # at Phase 4 (obstacle-free) for all 3 seeds and never saw a
                # dynamic obstacle. No regression: the point is guaranteed
                # exposure to Phases 5-7, not re-mastery of earlier phases.
                if (self._phase < 7
                        and self._phase_ep_count >= FORCED_PHASE_BUDGET.get(self._phase, 300)):
    ...
    def _save_phase(self):
        ...
        state = {
            ...
            # [Task 8 lstm_forced] MUST persist: this is the advancement
            # criterion here (not SR). Without it, a restart resets the counter
            # and the current phase silently gets its whole budget a second
            # time — so phases 5-7 may never be reached inside the 3000-episode
            # budget, defeating the entire purpose of this run.
            "phase_ep_count": self._phase_ep_count,
        }
    ...
    def _load_saved_phase(self) -> int:
        ...
        try:
            ...
            # [Task 8 lstm_forced] restore progress within the current phase
            self._phase_ep_count = state.get("phase_ep_count", 0)
            _budget = FORCED_PHASE_BUDGET.get(phase, 300)
            self.get_logger().info(
                f"[Resume] Phase {phase}, window={len(self._result_window)}, "
                f"eps={self._total_eps}, succs={self._total_succs}, "
                f"phase_progress={self._phase_ep_count}/{_budget}")
            return phase
        except Exception:
            try:
    ...
    def _advance_phase(self, sr: float):
        self._phase += 1
        self._phase_ep_count = 0  # [Task 8 lstm_forced] reset for the new phase's own budget
        self._pool     = list(PHASE_POOLS[self._phase])
        self._prev_idx = -1
