#!/usr/bin/env python3
"""
train_agent_sac_v8.py  —  SAC with STAM v2 (Temporal Multi-Head Attention)
==========================================================================
Upgrades over v7:
  1. Multi-head self-attention STAM (cross-sector reasoning)
  2. 3-frame temporal stacking (obstacle trajectory tracking)
  3. Prioritized Experience Replay (PER, SumTree)
  4. 3-step returns (faster credit assignment)
  5. 3-layer residual networks (actor + critic)
  6. Scheduled entropy target (exploration → exploitation)

Observation: env publishes 54 dims. We stack 3 LiDAR frames internally:
  scan_t (24) + scan_{t-1} (24) + scan_{t-2} (24) + nav (6) = 78 dims
  scan_vel (24:48) from env is dropped — temporal stack makes it redundant.

Usage:
  DZ_MODE=stam RUN_ID=sac_v8_stam FRESH=true ros2 run tb3_drl_nav train_agent_sac_v8
"""
...
OBS_DIM    = 24 * N_FRAMES + 6   # stacked obs: 78
N_SECTORS  = 24
...
class CriticV8(nn.Module):
    """Twin Q-networks with STAM v2 + residual block. No LayerNorm on critic."""
...
class SACv8:
    def __init__(self, device):
        self.device = device
        self.actor      = ActorV8().to(device)
        self.critic     = CriticV8().to(device)
        self.critic_tgt = CriticV8().to(device)
...
class TrainAgentSACv8(Node):
    def __init__(self):
        super().__init__("train_agent_sac_v8")
        self.declare_parameter("run_id", "sac_v8_stam")
        ...
        # Resume or fresh
        if self._fresh:
            self.get_logger().info("[SACv8] FRESH START — ignoring checkpoints.")
        else:
            self._resume()
        ...
        self.get_logger().info(
            f"[SACv8] Ready  run={run_id}  DZ_MODE={DZ_MODE}  mode={mode_str}  "
        ...
        if ckpt is None:
            if last_error is not None:
                self.get_logger().error(
                    f"[SACv8] Resume failed (no valid checkpoints: {last_error}) — fresh start.")
            return
        try:
            ...
            if len(self.replay) == 0:
                ...
                if os.path.exists(rb):
                    try:
                        ...
                        pass
            self.get_logger().info(
                f"[SACv8] Resumed ep={self._ep}  buf={len(self.replay)}  "
                f"upd={self._updates}  elapsed={self._elapsed_offset/3600:.1f}h")
        except Exception as e:
            self.get_logger().error(f"[SACv8] Resume failed ({e}) — fresh start.")
    ...
    def _shutdown(self, signum, frame):
        self.get_logger().info("[SACv8] Shutdown — saving with replay…")
        self.nstep.reset()
    ...
    def _on_phase(self, msg):
        ...
        if msg.data > old:
            self.get_logger().info(f"[SACv8] Phase {old} → {msg.data}")
    ...
    def _end_episode(self, info):
        ...
        self.get_logger().info(
            ...
            f"goal={self._cur_goal} | SR={sr:5.1f}% | H_tgt={self._target_entropy:.2f} | "
        ...
        if not EVAL_ONLY:
            ...
            if sr > self._best_sr and self._ep >= 100:
                ...
                self.get_logger().info(
                    f"[SACv8] *** NEW BEST SR={sr:.1f}% at ep={self._ep} ***")

        # Auto-stop
        if MAX_EPISODES > 0 and self._ep >= MAX_EPISODES:
            self.get_logger().info(
                f"[SACv8] MAX_EPISODES={MAX_EPISODES} reached — SR={sr:.1f}%")
            self._save_ckpt()
            rclpy.shutdown()
            return
