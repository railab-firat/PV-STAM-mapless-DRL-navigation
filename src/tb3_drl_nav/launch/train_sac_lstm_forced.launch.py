#!/usr/bin/env python3
"""train_sac_lstm_forced.launch.py — Task 8: LSTM with forced curriculum.
Identical structure to train_sac_lstm.launch.py — same sac_lstm agent,
unchanged — but paired with goal_manager_dynamic_forced (fixed per-phase
episode budget instead of the SR gate) so it actually reaches Phases 5-7."""
import os
...
def generate_launch_description():
    ...
    return LaunchDescription([
        DeclareLaunchArgument("run_id", default_value=f"sac_lstm_forced_s{seed}"),
        ...
        TimerAction(period=6.0, actions=[
            ...
            Node(package="tb3_drl_nav", executable="goal_manager_dynamic_forced",
                 parameters=[{"start_phase": 0}], output="screen"),
            Node(package="tb3_drl_nav", executable="obstacle_controller", output="screen"),
            Node(package="tb3_drl_nav", executable="sac_lstm",
